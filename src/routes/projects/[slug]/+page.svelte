<script>
	...
	function getWidth(image) {
		...
		const selected = aspectRatio >= 1.2 ? wide : tall;
		...
		{#each data.images as image}
			<div class="relative aspect-[3/4] overflow-hidden">
				<div class="absolute inset-0 flex items-center justify-center">
					<button onclick={() => (displayImage = image)}>
						<img
							src={image.url}
							alt=""
							loading="lazy"
							class="absolute cursor-pointer opacity-0 transition-all duration-300"
							onload={(e) => {
...
{#if displayImage != null}
	...
	>
		...
		<img src={displayImage.url} alt="" class="max-h-[80vh] max-w-[90vw] object-contain" />
	</div>
{/if}
